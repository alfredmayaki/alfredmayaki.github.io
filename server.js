...
// Serve static site (put your static files like msc-survey.html in ./public/)
...
app.post('/api/gpt', async (req, res) => {
  ...
  try {
    ...
    // If client requested a streamed response, proxy OpenAI's stream
    const wantsStream = (req.query && req.query.stream) || (req.headers.accept && req.headers.accept.includes('text/event-stream'));
    ...
    if (wantsStream) {
      ...
      // Set headers for SSE-like streaming to browser
      res.setHeader('Content-Type', 'text/event-stream');
      res.setHeader('Cache-Control', 'no-cache');
      res.setHeader('Connection', 'keep-alive');
      res.flushHeaders && res.flushHeaders();

      // Pipe upstream stream chunks directly to client
      ...
      return; // response handled by streaming
    }

    // Non-streaming fallback (legacy behavior)
    if (isChatModel) {
      const r = await fetch('https://api.openai.com/v1/chat/completions', {
        ...
        body: JSON.stringify({ model: usedModel, messages: [{ role: 'user', content: prompt }], max_tokens: 800, temperature: 0.2 })
      });
    ...
    const r = await fetch('https://api.openai.com/v1/completions', {
      ...
      body: JSON.stringify({ model: usedModel, prompt, max_tokens: 600, temperature: 0.2 })
    });
